...
# Usage: .\Analyze-EventTicketTypes.ps1 -EventName "Event Name" -StripeSecretKey "sk_live_..."
...
if ([string]::IsNullOrEmpty($StripeSecretKey)) {
    ...
    if (Test-Path $configPath) {
        ...
        Write-Error "Stripe secret key not provided and config file not found at $configPath"
        exit 1
    }
}
...
Write-Host "1. Fetching checkout sessions from Stripe..." -ForegroundColor Yellow
$allSessions = @()
$hasMore = $true
$startingAfter = $null

try {
    while ($hasMore -and $allSessions.Count -lt $MaxSessions) {
...
Write-Host ""
Write-Host "2. Filtering for '$EventName' events..." -ForegroundColor Yellow
...
$processingFeeConfig = $null
$afterPayFeeConfig = $null
if (Test-Path $configPath) {
...
Write-Host ""
Write-Host "3. Analyzing ticket types..." -ForegroundColor Yellow
...
if ($ExportCSV -or -not [string]::IsNullOrEmpty($ExportPath)) {
    Write-Host ""
    Write-Host "4. Exporting data to CSV files..." -ForegroundColor Yellow
