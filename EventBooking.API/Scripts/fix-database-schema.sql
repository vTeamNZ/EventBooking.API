-- Database Schema Fix Script for KiwiLanka Event Booking System
-- This script fixes the column mismatches and cleans up the database
...
-- Step 2: Fix Seats table - Add TicketTypeId column if it doesn't exist
...
-- Step 3: Fix Events table - ensure Name column is renamed to Title if it exists
...
ELSE
BEGIN
    PRINT 'No Name column found in Events table - good!';
END

-- Step 4: Ensure TicketTypes table has Name column
...
-- Step 5: Drop foreign key constraint on SectionId if it exists
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME LIKE '%Seats_SectionId%')
BEGIN
    DECLARE @constraintName NVARCHAR(255);
    SELECT @constraintName = CONSTRAINT_NAME 
    FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS 
    WHERE CONSTRAINT_NAME LIKE '%Seats_SectionId%';
    
    EXEC('ALTER TABLE Seats DROP CONSTRAINT ' + @constraintName);
    PRINT 'Dropped foreign key constraint on SectionId';
END

-- Step 6: Drop SectionId column from Seats if it exists
...
-- Step 7: Drop Sections table if it exists
...
-- Step 8: Add foreign key constraint for TicketTypeId
...
-- Create some sample seats for the first two events
DECLARE @EventId INT = 1;
DECLARE @Row INT = 1;
DECLARE @SeatNum INT = 1;
DECLARE @TicketTypeId INT;

-- VIP Seats for Event 1 (Rows 1-5, 40 seats per row)
...
-- Premium Seats for Event 1 (Rows 6-10 only - to keep it manageable)
...
-- Add some seats for Event 2 (Comedy Night) - Front rows only
SET @EventId = 2;
SET @Row = 1;
